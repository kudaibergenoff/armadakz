<div class="user-block d-flex">
    ...
    <form action="{{ route('user.personal') }}" class="user-block__wrap" method="POST">
        @csrf
            ...
            <ul class="user-block__items row">
                <li class="user-block-info user-block__item col-12 col-md-6 col-xl-4">
                    ...
                    <div class="user-block-info__value">
                        <input type="text" class="form-control" name="sname" value="{{ $info->sname ?? old('sname') }}" required>
                        <span>{{ $info->sname ?? 'Не указано' }}</span>
                    </div>
                </li>
                <li class="user-block-info user-block__item col-12 col-md-6 col-xl-4">
                    <div class="user-block-info__name">Имя</div>
                    <div class="user-block-info__value">
                        <input type="text" class="form-control" name="name" value="{{ $info->name ?? old('name') }}" required>
                        <span>{{ $info->name ?? 'Не указано' }}</span>
                    </div>
                </li>
                <li class="user-block-info user-block__item col-12 col-md-6 col-xl-4">
                    <div class="user-block-info__name">Отчество</div>
                    <div class="user-block-info__value">
                        <input type="text" class="form-control" name="mname" value="{{ $info->mname ?? old('mname') }}">
                        <span>{{ $info->mname ?? 'Не указано' }}</span>
                    </div>
                </li>
                <li class="user-block-info user-block__item col-12 col-md-6 col-xl-4">
                    <div class="user-block-info__name">Дата рождения</div>
                    <div class="user-block-info__value">
                        <input type="text" name="dob" id="dob" data-toggle="datepicker" class="form-control docs-date" value="{{ $info->dob ?? old('dob') }}" placeholder="дд.мм.гггг" required>
                        <span>{{ $info->dob ?? 'Не указан' }}</span>
                    </div>
                </li>
                <li class="user-block-info user-block__item col-12 col-md-6 col-xl-4">
                    ...
                    <div class="user-block-info__value">
                        ...
                        <div>
                            <select class="mdb-select md-form m-0" name="sex_id">
                                @foreach($sexes as $sex)
                                    <option value="{{ $sex->id }}" @if($info->sex_id == $sex->id or old('sex_id') == $sex->id or ($info->sex_id == null and $loop->first)) selected @endif>{{ $sex->title_ru }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <li class="user-block-info user-block__item col-12 col-md-6 col-xl-4">
                    ...
                    <div class="user-block-info__value">
                        <div>
                            <select class="mdb-select md-form m-0" name="lang_id" searchable="Поиск...">
                                @foreach($langs as $lang)
                                    <option value="{{ $lang->id }}" @if($info->lang_id == $lang->id or old('lang_id') == $lang->id or ($info->lang_id == null and $loop->first)) selected @endif>{{ $lang->title_ru }}</option>
                                @endforeach
                            </select>
                        </div>
                        <span>{{ $info->lang->title_ru ?? 'Не выбран' }}</span>
                    </div>
                </li>
            </ul>
        </div>
