        ...
        <form action="{{ route('user.address') }}" class="user-block__wrap" method="POST">
            ...
            <div class="user-block__content">
                <ul class="user-block__items row">
                    <li class="user-block-info user-block__item col-12 col-md-6">
                        <div class="user-block-info__name">Страна</div>
                        <div class="user-block-info__value">
                            <div>
                                <span class="d-block mt-2">{{ $user->city->country->title_ru ?? 'Казахстан' }}</span>
                            </div>
                            <span>{{ $user->city->country->title_ru ?? 'Казахстан' }}</span>
                        </div>
                    </li>
                    <li class="user-block-info user-block__item col-12 col-md-6">
                        ...
                        <div class="user-block-info__value">
                            <div>
                                <select class="form-control m-0" name="city_id" searchable="Поиск...">
                                    @foreach($cities as $city)
                                        <option value="{{ $city->id }}" @if($user->city_id == $city->id or old('city_id') == $city->id or ($user->city_id == null and $loop->first)) selected @endif>{{ $city->title_ru }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <span>{{ $user->city->title_ru ?? 'Не выбран' }}</span>
                        </div>
                    </li>
                    <li class="user-block-info user-block__item col-12">
