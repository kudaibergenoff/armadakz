...
@section('title','ARMADA - Смена пароля')
...
@section('content')
    ...
    <div class="page-login__bg" style="background: url('{{ asset('img/sections-bg/register-bg.jpg') }}')">
        ...
        <section class="login page-login__login">
            <form class="flex-column justify-content-between h-100" method="POST" action="{{ route('password.update') }}">
                @csrf

                <input type="hidden" name="token" value="{{ $token }}">

                <div>
                    <h3 class="h4 text-center login__title">{{ __('Сбросить пароль') }}</h3>
                    ...
                    <div class="md-form md-outline input-with-pre-icon">
                        ...
                        <input type="email" name="email" id="forgot-login" value="{{ $email ?? old('email') }}" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" required autofocus>
