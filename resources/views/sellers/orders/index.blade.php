...
@section('content')
    ...
    <section class="vendor page-vendor-area__vendor container-fluid pl-0">
        <div class="d-flex">
            @include('sellers.orders._filters')
                ...
                <div class="products orders vendor__products pt-4 pb-3 p-xxl-3 pt-xxl-5 pr-0">
                    <div class="orders__header justify-content-between">
                        <div class="d-flex align-content-center flex-wrap">
                            <h2 class="page-title orders__title mr-4 mb-2">Все заказы</h2>
                            <button class="button btn-sm btn-light products__filter mr-2 mb-2">Фильтр</button>
                            @if(isset($_GET) and count($_GET) > 0)
                                <a href="{{ route('seller.orders.index') }}" class="text-primary">
                                    <i class="fas fa-filter"></i>
                                    Сбросить фильтр
                                </a>
                            @else
                                заказы со статусом "в ожидании" за последний месяц
                            @endif
                        </div>
                        <span class="orders__count">Найдено <b>{{ $orders->count() }}</b><!-- по фильтру --></span>
                    </div>
...
{{--                                    <th>Раздел</th>--}}
                            ...
                            <tbody>
                                @foreach($orders as $order)
                                    @include('sellers.orders._table_orders')
                                @endforeach
                            </tbody>
                        </table>
                    </div>
...
@push('scripts')
    ...
    @include('admin.orders._datepicker')
    <script>
        // Edit product
