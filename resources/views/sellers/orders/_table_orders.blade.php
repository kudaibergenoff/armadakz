<tr>
    ...
    <td>
        @if($order->user_id != null) <i class="fas fa-user-tie"></i> {{ $order->userInfo->fio() }}<br> @endif
        @if($order->fio != null) <i class="far fa-user"></i>{{ $order->fio }}<br> @endif
        @if($order->phone != null) <i class="fas fa-mobile-alt"></i> {{ $order->phone }}<br> @endif
        @if($order->country_id != null) <i class="fas fa-globe"></i> {{ $order->country->title_ru }}<br> @endif
        @if($order->city_id != null) <i class="fas fa-map-marked-alt"></i> {{ $order->city->title_ru }}<br> @endif
        @if($order->address != null) <i class="fas fa-map-marker-alt"></i> {{ $order->address }}<br> @endif
        @if($order->comment != null) <i class="far fa-comment-dots"></i> {{ $order->comment }}<br> @endif
    </td>
    <td class="products-table__image"><img src="{{ $order->getImage() }}" width="50" alt="Product image"></td>
    <td class="d-flex flex-column align-items-start">
        <div class="products__status products__status--{{ $order->orderStatus->color ?? 'hot' }}">{{ $order->orderStatus->title_ru }}</div>
    </td>
    <td>{{ $order->articul }}</td>
    <td>
        @if(isset($order->product->slug) and $order->product->slug != null)
            <a href="{{ route('product',[$order->product->id, $order->product->slug]) }}" target="_blank">{{ $order->title }}</a>
        @else
            <span class="text-danger font-weight-bold">Нет данных!!!</span>
        @endif
    </td>
{{--    <td>Мебельная фурнитура</td>--}}
    <td>
        <span class="font-weight-bold">{!! $order->product->item->title !!}</span><br>
        <span class="mx-2"><i class="fas fa-angle-double-right"></i></span>
        {!! $order->product->item->subcatalog->title !!}<br>
        <span class="mx-2"><i class="fas fa-angle-double-right"></i></span>
        <span class="text-muted">{!! $order->product->item->subcatalog->catalog->title !!}</span>
    </td>
    <td>
        @if(isset($order->store->slug) and $order->store->slug != null)
            <a href="{{ route('shop',$order->store->slug) }}" target="_blank">{{ $order->store_title }}</a>
        @else
            <span class="text-danger font-weight-bold">Нет данных!!!</span>
        @endif
    </td>
    <td><span class="font-weight-semibold products-table__price">{{ $order->price }}</span></td>
    <td>
        <b>Создания:</b><br>
    ...
    <td class="text-center">
        <a href="{{ route('seller.orders.edit',['id'=>$order->id]) }}" class="products-table__edit" data-item-id="{{ $order->id }}">
            <input type="hidden" name="edit-action" value="{{ route('seller.orders.edit',['id'=>$order->id]) }}">
            <input type="hidden" name="edit-link" value="{{ route('seller.orders.update',['id'=>$order->id]) }}">
            <input type="hidden" name="edit-product-status" value="{{ $order->status_id }}">
        ...
        <form action="{{ route('seller.orders.update',['id'=>$order->id]) }}" method="POST" enctype="multipart/form-data" class="change-order-status">
            @method('PATCH')
            @csrf
            <select name="status_id" class="mdb-select md-form mx-3 my-0">
                <option value="0" selected disabled>Изменить статус</option>
                @foreach($statuses as $status)
                    <option value="{{ $status->id }}" @if($order->status_id == $status->id) selected @endif>{{ $status->title_ru }}</option>
                @endforeach
            </select>
        </form>
{{--        <button class="products-table__delete mr-xxl-2">--}}
{{--            <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">--}}
