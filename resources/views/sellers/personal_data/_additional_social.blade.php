<div class="duplicate user-block__additional-phones bg-light rounded py-3 px-4 mt-4">
    <p class="mb-4">Другие виды связи</p>

    @if(isset($user->additional_social) and count($user->additional_social) > 0)
        @for($i = 0; $i < count($user->additional_social); $i++)
            <div class="duplicate__item user-block__additional-phone d-flex align-items-center" data-duplicate-item-id="1">
                <ul class="user-block__items flex-grow-1 row user-block__additional-other">
                    <li class="user-block-info user-block__item col-12 col-md-6 mb-0 col-xl-4">
                        <div class="user-block-info__name">Тип связи</div>
                        <div class="user-block-info__value">
                            <select class="custom-select m-0" name="additional_social[]">
                                <option value="" selected>Выберите из списка</option>
                                @foreach(['Viber','Telegram','WhatsApp','Messenger','Site','Skype','ICQ'] as $messenger)
                                    <option value="{{ $messenger }}" @if($user->additional_social[$i] == $messenger) selected @endif>{{ $messenger }}</option>
                                @endforeach
                            </select>
                            <span>{{ $user->additional_social[$i] ?? 'Не указан' }}</span>
                        </div>
                    </li>
                    <li class="user-block-info user-block__item col-12 col-md-6 mb-0 col-xl-4">
                        <div class="user-block-info__name">Значение</div>
                        <div class="user-block-info__value">
                            <input type="text" class="form-control" name="additional_social_value[]" value="{{ $user->additional_social_value[$i] ?? old('additional_social_value') }}">
                            <span>{{ $user->additional_social_value[$i] ?? 'Не указан' }}</span>
                        </div>
                    </li>
                </ul>
        ...
        <div class="duplicate__item user-block__additional-phone d-flex align-items-center" data-duplicate-item-id="1">
            <ul class="user-block__items flex-grow-1 row user-block__additional-other">
                ...
                <li class="user-block-info user-block__item col-12 col-md-6 mb-0 col-xl-4">
                    <div class="user-block-info__name">Значение</div>
                    <div class="user-block-info__value">
                        <input type="text" class="form-control" name="additional_social_value[]">
                        <span>Не указан</span>
                    </div>
                </li>
            </ul>
