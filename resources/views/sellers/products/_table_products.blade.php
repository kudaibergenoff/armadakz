<tr>
    ...
    <td>{!! $product->articul ?? '<span class="text-danger font-weight-bold">нет данных !!!</span>' !!}</td>
    <td>{{ $product->title }}</td>
{{--    <td>{!! $product->catalog->title !!}</td>--}}
    <td>
        <div class="d-inline-flex white-space-nowrap">
        ...
        <span class="text-muted white-space-nowrap">{{ $product->item->subcatalog->catalog->title }}</span><br>
    </td>
    <td>
        {!! $product->store->title !!}
    </td>
...
{{--        <div class="products__status products__status--five">В пятерку</div>--}}
    ...
    <td class="text-center">
        <a href="{{ route('seller.products.edit',['id'=>$product->id]) }}" class="products-table__edit mx-1"> <!--  data-toggle="modal" data-target="#quick-edit" data-item-id="{{ $product->id }}" -->
{{--            <input type="hidden" name="edit-action" value="{{ route('seller.products.edit',['id'=>$product->id]) }}">--}}
{{--            <input type="hidden" name="edit-link" value="{{ route('seller.productUpdate',['id'=>$product->id]) }}">--}}
...
{{--            <input type="hidden" name="edit-product-price" value="{{ $product->price }}">--}}
        ...
        <button class="products-table__delete mx-1" data-toggle="modal" data-target="#confirm-delete-modal" data-item-id="{{ $product->id }}">
            <input type="hidden" name="delete-action" value="{{ route('seller.products.destroy',['id'=>$product->id]) }}">
