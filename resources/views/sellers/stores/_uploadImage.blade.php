<div class="images-wrap">
{{--    <div class="image-upload-wrap d-none">--}}
{{--        <div class="image-upload">--}}
{{--            <div class="image-upload__preview" data-default-image="{{ asset('/img/noimg.jpg') }}" style="background-image: url({{ asset('/img/noimg.jpg') }})"></div>--}}
{{--            <input type="hidden" name="{{ $name }}" value="">--}}
{{--            <ul class="image-upload__actions">--}}
{{--                <li class="image-upload__action image-upload__action--load">--}}
{{--                    <label>--}}
{{--                        <svg width="21" height="25" viewBox="0 0 21 25" fill="none" xmlns="http://www.w3.org/2000/svg">--}}
{{--                            <path d="M5.98606 6.98194L9.44853 3.54907V18.7546C9.44853 19.33 9.91959 19.7971 10.5 19.7971C11.0804 19.7971 11.5515 19.33 11.5515 18.7546V3.54907L15.0139 6.98194C15.219 7.18574 15.4881 7.28738 15.7573 7.28738C16.0265 7.28738 16.2957 7.18574 16.5007 6.98194C16.9113 6.57485 16.9113 5.91497 16.5007 5.50788L11.2434 0.295514C10.8333 -0.111571 10.1667 -0.111571 9.75661 0.295514L4.49929 5.50788C4.0887 5.91497 4.0887 6.57485 4.49929 6.98194C4.90936 7.38902 5.57599 7.38902 5.98606 6.98194Z" fill="white"/>--}}
{{--                            <path d="M19.9632 16.6699C19.3827 16.6699 18.9117 17.1369 18.9117 17.7124V21.8823C18.9117 22.4572 18.4401 22.9247 17.8602 22.9247H3.13974C2.55986 22.9247 2.08828 22.4572 2.08828 21.8823V17.7124C2.08828 17.1369 1.61722 16.6699 1.03681 16.6699C0.456407 16.6699 -0.0146484 17.1369 -0.0146484 17.7124V21.8823C-0.0146484 23.607 1.4001 25.0097 3.13974 25.0097H17.8602C19.5999 25.0097 21.0146 23.607 21.0146 21.8823V17.7124C21.0146 17.1369 20.5436 16.6699 19.9632 16.6699Z" fill="white"/>--}}
{{--                        </svg>--}}
{{--                        <input--}}
{{--                            type="file"--}}
{{--                            class="image-upload__upload img"--}}
{{--                            value="Загрузить фото"--}}
{{--                            accept=".png, .jpg, .jpeg"--}}
{{--                            @isset($aspectRatio)--}}
{{--                            data-aspect-ratio="{{ $aspectRatio }}"--}}
{{--                            @endisset>--}}
{{--                    </label>--}}
{{--                </li>--}}
{{--                <li class="image-upload__action image-upload__action--delete">--}}
{{--                    <svg width="23" height="25" viewBox="0 0 23 25" fill="none" xmlns="http://www.w3.org/2000/svg">--}}
{{--                        <path d="M14.9484 9.05753C14.625 9.05753 14.3629 9.31963 14.3629 9.64301V20.7086C14.3629 21.0317 14.625 21.2941 14.9484 21.2941C15.2718 21.2941 15.5338 21.0317 15.5338 20.7086V9.64301C15.5338 9.31963 15.2718 9.05753 14.9484 9.05753Z" fill="white" stroke="white" stroke-width="0.75"/>--}}
{{--                        <path d="M8.03973 9.05753C7.71635 9.05753 7.45425 9.31963 7.45425 9.64301V20.7086C7.45425 21.0317 7.71635 21.2941 8.03973 21.2941C8.36312 21.2941 8.62521 21.0317 8.62521 20.7086V9.64301C8.62521 9.31963 8.36312 9.05753 8.03973 9.05753Z" fill="white" stroke="white" stroke-width="0.75"/>--}}
{{--                        <path d="M3.00462 7.44266V21.8676C3.00462 22.7202 3.31726 23.5209 3.8634 24.0954C4.40702 24.6715 5.16357 24.9986 5.95534 24.9999H17.0328C17.8248 24.9986 18.5813 24.6715 19.1247 24.0954C19.6709 23.5209 19.9835 22.7202 19.9835 21.8676V7.44266C21.0691 7.1545 21.7726 6.10567 21.6274 4.99166C21.4819 3.87788 20.5331 3.04472 19.4097 3.04449H16.4121V2.31264C16.4155 1.6972 16.1722 1.10623 15.7365 0.67147C15.3008 0.236935 14.7089 -0.00503178 14.0935 -3.20557e-07H8.89462C8.27918 -0.00503178 7.6873 0.236935 7.25162 0.67147C6.81595 1.10623 6.57261 1.6972 6.57604 2.31264V3.04449H3.57843C2.45505 3.04472 1.50616 3.87788 1.36071 4.99166C1.21548 6.10567 1.91897 7.1545 3.00462 7.44266V7.44266ZM17.0328 23.829H5.95534C4.95431 23.829 4.17558 22.969 4.17558 21.8676V7.49412H18.8125V21.8676C18.8125 22.969 18.0338 23.829 17.0328 23.829ZM7.74699 2.31264C7.74311 2.00778 7.86295 1.71435 8.0793 1.49914C8.29542 1.28394 8.58953 1.1657 8.89462 1.17096H14.0935C14.3986 1.1657 14.6927 1.28394 14.9088 1.49914C15.1252 1.71412 15.245 2.00778 15.2411 2.31264V3.04449H7.74699V2.31264ZM3.57843 4.21544H19.4097C19.9917 4.21544 20.4635 4.68726 20.4635 5.26931C20.4635 5.85135 19.9917 6.32317 19.4097 6.32317H3.57843C2.99639 6.32317 2.52457 5.85135 2.52457 5.26931C2.52457 4.68726 2.99639 4.21544 3.57843 4.21544V4.21544Z" fill="white" stroke="white" stroke-width="0.75"/>--}}
{{--                        <path d="M11.4941 9.05753C11.1707 9.05753 10.9086 9.31963 10.9086 9.64301V20.7086C10.9086 21.0317 11.1707 21.2941 11.4941 21.2941C11.8174 21.2941 12.0795 21.0317 12.0795 20.7086V9.64301C12.0795 9.31963 11.8174 9.05753 11.4941 9.05753Z" fill="white" stroke="white" stroke-width="0.75"/>--}}
{{--                    </svg>--}}
{{--                </li>--}}
{{--                <li class="image-upload__action image-upload__action--clear">--}}
{{--                    <svg width="29" height="25" viewBox="0 0 29 25" fill="none" xmlns="http://www.w3.org/2000/svg">--}}
{{--                        <path d="M28.1504 23.2497H18.8379L27.8358 14.0983C28.5861 13.3353 28.9993 12.3209 28.9993 11.2419C28.9993 10.163 28.5861 9.14851 27.8358 8.38549L20.7742 1.20381C19.2255 -0.371247 16.7056 -0.371247 15.1569 1.20381L1.16351 15.4352C0.413193 16.1982 0 17.2126 0 18.2917C0 19.3706 0.413193 20.385 1.16351 21.1481L4.68044 24.7248C4.83977 24.8868 5.05586 24.9779 5.28117 24.9779H28.1504C28.6196 24.9779 29 24.591 29 24.1138C29 23.6366 28.6196 23.2497 28.1504 23.2497ZM13.3633 5.47178L15.5993 7.7458L8.74673 14.715L6.51073 12.441L13.3633 5.47178ZM20.2017 12.4265L13.3491 19.3956L9.94819 15.9369L16.8008 8.96775L20.2017 12.4265ZM21.4032 13.6484L23.6392 15.9225L21.3471 18.2538L16.7867 22.8916L14.5506 20.6175L21.4032 13.6484ZM16.3584 2.42576C17.2446 1.52455 18.6865 1.52455 19.5727 2.42576L26.6343 9.60744C27.5205 10.5087 27.5205 11.9752 26.6343 12.8764L24.8408 14.7005L14.5648 4.24984L16.3584 2.42576ZM2.36497 19.9261C1.93564 19.4895 1.69922 18.9089 1.69922 18.2917C1.69922 17.6743 1.93564 17.0938 2.36492 16.6572L5.30915 13.6629L8.14583 16.5478L8.14588 16.5479L12.7482 21.2285C12.7484 21.2287 12.7486 21.2288 12.7487 21.229L14.7357 23.2497H5.63308L2.36497 19.9261Z" fill="white"/>--}}
{{--                    </svg>--}}
{{--                </li>--}}
{{--            </ul>--}}
{{--        </div>--}}
{{--    </div>--}}
    @if(isset($data) and $data->$name != null and count($data->$name) > 0)
        @foreach($data->$name as $image)
            @isset($label)
                <label class="d-flex align-items-center">
                    <span>{{ $label }}</span>
                    @isset($tooltip)
                        <span class="material-tooltip-main ml-3 d-inline-flex" data-toggle="tooltip" data-tippy-content="{{ $tooltip }}">
                        <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M11 22C8.0618 22 5.29947 20.8558 3.2218 18.7782C1.14421 16.7005 0 13.9382 0 11C0 8.0618 1.14421 5.29947 3.2218 3.2218C5.29947 1.14421 8.0618 0 11 0C13.9382 0 16.7005 1.14421 18.7782 3.2218C20.8558 5.29947 22 8.0618 22 11C22 13.9382 20.8558 16.7005 18.7782 18.7782C16.7005 20.8558 13.9382 22 11 22Z" fill="#E0001A" fill-opacity="0.72"/>
                            <path d="M18.7782 3.2218C16.7005 1.14421 13.9382 0 11 0V22C13.9382 22 16.7005 20.8558 18.7782 18.7782C20.8558 16.7005 22 13.9382 22 11C22 8.0618 20.8558 5.29947 18.7782 3.2218Z" fill="#E0001A"/>
                            <path d="M12.9336 15.9414V9.49609H8.20703V10.7852H9.06641V15.9414H8.16406V17.2305H13.75V15.9414H12.9336Z" fill="#E4F7FF"/>
                            <path d="M11 8.20703C12.0662 8.20703 12.9336 7.33962 12.9336 6.27344C12.9336 5.20725 12.0662 4.33984 11 4.33984C9.93382 4.33984 9.06641 5.20725 9.06641 6.27344C9.06641 7.33962 9.93382 8.20703 11 8.20703Z" fill="#E4F7FF"/>
                            <path d="M11 8.20703C12.0662 8.20703 12.9336 7.33962 12.9336 6.27344C12.9336 5.20725 12.0662 4.33984 11 4.33984V8.20703Z" fill="#E0001A" fill-opacity="0.07"/>
                            <path d="M12.9336 15.9414V9.49609H11V17.2305H13.75V15.9414H12.9336Z" fill="#E0001A" fill-opacity="0.07"/>
                        </svg>
                    </span>
                    @endisset
                </label>
            @endisset
                <div class="image-upload-wrap">
                    <div class="image-upload">
                        <div class="image-upload__preview" data-default-image="{{ asset('/img/noimg.jpg') }}" style="background-image: url({{ $data->getImages(0,$name,'/img/noimg.jpg') ?? asset('/img/noimg.jpg') }})"></div>
                        <input type="hidden" name="{{ $name }}" value="{{ $image }}">
                        <ul class="image-upload__actions">
                            <li class="image-upload__action image-upload__action--load">
                                <label>
                                    <svg width="21" height="25" viewBox="0 0 21 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M5.98606 6.98194L9.44853 3.54907V18.7546C9.44853 19.33 9.91959 19.7971 10.5 19.7971C11.0804 19.7971 11.5515 19.33 11.5515 18.7546V3.54907L15.0139 6.98194C15.219 7.18574 15.4881 7.28738 15.7573 7.28738C16.0265 7.28738 16.2957 7.18574 16.5007 6.98194C16.9113 6.57485 16.9113 5.91497 16.5007 5.50788L11.2434 0.295514C10.8333 -0.111571 10.1667 -0.111571 9.75661 0.295514L4.49929 5.50788C4.0887 5.91497 4.0887 6.57485 4.49929 6.98194C4.90936 7.38902 5.57599 7.38902 5.98606 6.98194Z" fill="white"/>
                                        <path d="M19.9632 16.6699C19.3827 16.6699 18.9117 17.1369 18.9117 17.7124V21.8823C18.9117 22.4572 18.4401 22.9247 17.8602 22.9247H3.13974C2.55986 22.9247 2.08828 22.4572 2.08828 21.8823V17.7124C2.08828 17.1369 1.61722 16.6699 1.03681 16.6699C0.456407 16.6699 -0.0146484 17.1369 -0.0146484 17.7124V21.8823C-0.0146484 23.607 1.4001 25.0097 3.13974 25.0097H17.8602C19.5999 25.0097 21.0146 23.607 21.0146 21.8823V17.7124C21.0146 17.1369 20.5436 16.6699 19.9632 16.6699Z" fill="white"/>
                                    </svg>
                                    <input
                                        type="file"
                                        class="image-upload__upload img"
                                        value="Загрузить фото"
                                        accept=".png, .jpg, .jpeg"
                                        @isset($aspectRatioX)
                                            data-aspect-ratio-x="{{ $aspectRatioX }}"
                                        @endisset
                                        @isset($aspectRatioY)
                                            data-aspect-ratio-y="{{ $aspectRatioY }}"
                                        @endisset
                                    >
                                </label>
                            </li>
                            <li class="image-upload__action image-upload__action--clear">
                                <svg width="29" height="25" viewBox="0 0 29 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M28.1504 23.2497H18.8379L27.8358 14.0983C28.5861 13.3353 28.9993 12.3209 28.9993 11.2419C28.9993 10.163 28.5861 9.14851 27.8358 8.38549L20.7742 1.20381C19.2255 -0.371247 16.7056 -0.371247 15.1569 1.20381L1.16351 15.4352C0.413193 16.1982 0 17.2126 0 18.2917C0 19.3706 0.413193 20.385 1.16351 21.1481L4.68044 24.7248C4.83977 24.8868 5.05586 24.9779 5.28117 24.9779H28.1504C28.6196 24.9779 29 24.591 29 24.1138C29 23.6366 28.6196 23.2497 28.1504 23.2497ZM13.3633 5.47178L15.5993 7.7458L8.74673 14.715L6.51073 12.441L13.3633 5.47178ZM20.2017 12.4265L13.3491 19.3956L9.94819 15.9369L16.8008 8.96775L20.2017 12.4265ZM21.4032 13.6484L23.6392 15.9225L21.3471 18.2538L16.7867 22.8916L14.5506 20.6175L21.4032 13.6484ZM16.3584 2.42576C17.2446 1.52455 18.6865 1.52455 19.5727 2.42576L26.6343 9.60744C27.5205 10.5087 27.5205 11.9752 26.6343 12.8764L24.8408 14.7005L14.5648 4.24984L16.3584 2.42576ZM2.36497 19.9261C1.93564 19.4895 1.69922 18.9089 1.69922 18.2917C1.69922 17.6743 1.93564 17.0938 2.36492 16.6572L5.30915 13.6629L8.14583 16.5478L8.14588 16.5479L12.7482 21.2285C12.7484 21.2287 12.7486 21.2288 12.7487 21.229L14.7357 23.2497H5.63308L2.36497 19.9261Z" fill="white"/>
                                </svg>
                            </li>
                        </ul>
                    </div>
                </div>
        @endforeach
    @else
        @isset($label)
            <label>{{ $label }}</label>
            @isset($tooltip)
                <span class="material-tooltip-main ml-3 d-inline-flex" data-toggle="tooltip" data-tippy-content="{{ $tooltip }}">
                        <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M11 22C8.0618 22 5.29947 20.8558 3.2218 18.7782C1.14421 16.7005 0 13.9382 0 11C0 8.0618 1.14421 5.29947 3.2218 3.2218C5.29947 1.14421 8.0618 0 11 0C13.9382 0 16.7005 1.14421 18.7782 3.2218C20.8558 5.29947 22 8.0618 22 11C22 13.9382 20.8558 16.7005 18.7782 18.7782C16.7005 20.8558 13.9382 22 11 22Z" fill="#E0001A" fill-opacity="0.72"/>
                            <path d="M18.7782 3.2218C16.7005 1.14421 13.9382 0 11 0V22C13.9382 22 16.7005 20.8558 18.7782 18.7782C20.8558 16.7005 22 13.9382 22 11C22 8.0618 20.8558 5.29947 18.7782 3.2218Z" fill="#E0001A"/>
                            <path d="M12.9336 15.9414V9.49609H8.20703V10.7852H9.06641V15.9414H8.16406V17.2305H13.75V15.9414H12.9336Z" fill="#E4F7FF"/>
                            <path d="M11 8.20703C12.0662 8.20703 12.9336 7.33962 12.9336 6.27344C12.9336 5.20725 12.0662 4.33984 11 4.33984C9.93382 4.33984 9.06641 5.20725 9.06641 6.27344C9.06641 7.33962 9.93382 8.20703 11 8.20703Z" fill="#E4F7FF"/>
                            <path d="M11 8.20703C12.0662 8.20703 12.9336 7.33962 12.9336 6.27344C12.9336 5.20725 12.0662 4.33984 11 4.33984V8.20703Z" fill="#E0001A" fill-opacity="0.07"/>
                            <path d="M12.9336 15.9414V9.49609H11V17.2305H13.75V15.9414H12.9336Z" fill="#E0001A" fill-opacity="0.07"/>
                        </svg>
                    </span>
            @endisset
        @endisset
        <div class="image-upload-wrap">
            <div class="image-upload">
                <div class="image-upload__preview" data-default-image="{{ asset('/img/noimg.jpg') }}" style="background-image: url({{ asset('/img/noimg.jpg') }})"></div>
                <input type="hidden" name="@isset($name){{ $name }}@else images[] @endisset" value="">
                <ul class="image-upload__actions">
                    <li class="image-upload__action image-upload__action--load">
                        <label>
                            <svg width="21" height="25" viewBox="0 0 21 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M5.98606 6.98194L9.44853 3.54907V18.7546C9.44853 19.33 9.91959 19.7971 10.5 19.7971C11.0804 19.7971 11.5515 19.33 11.5515 18.7546V3.54907L15.0139 6.98194C15.219 7.18574 15.4881 7.28738 15.7573 7.28738C16.0265 7.28738 16.2957 7.18574 16.5007 6.98194C16.9113 6.57485 16.9113 5.91497 16.5007 5.50788L11.2434 0.295514C10.8333 -0.111571 10.1667 -0.111571 9.75661 0.295514L4.49929 5.50788C4.0887 5.91497 4.0887 6.57485 4.49929 6.98194C4.90936 7.38902 5.57599 7.38902 5.98606 6.98194Z" fill="white"/>
                                <path d="M19.9632 16.6699C19.3827 16.6699 18.9117 17.1369 18.9117 17.7124V21.8823C18.9117 22.4572 18.4401 22.9247 17.8602 22.9247H3.13974C2.55986 22.9247 2.08828 22.4572 2.08828 21.8823V17.7124C2.08828 17.1369 1.61722 16.6699 1.03681 16.6699C0.456407 16.6699 -0.0146484 17.1369 -0.0146484 17.7124V21.8823C-0.0146484 23.607 1.4001 25.0097 3.13974 25.0097H17.8602C19.5999 25.0097 21.0146 23.607 21.0146 21.8823V17.7124C21.0146 17.1369 20.5436 16.6699 19.9632 16.6699Z" fill="white"/>
                            </svg>
                            <input
                                type="file"
                                class="image-upload__upload img"
                                value="Загрузить фото"
                                accept=".png, .jpg, .jpeg"
                                @isset($aspectRatioX)
                                    data-aspect-ratio-x="{{ $aspectRatioX }}"
                                @endisset
                                @isset($aspectRatioY)
                                    data-aspect-ratio-y="{{ $aspectRatioY }}"
                                @endisset
                            >
                        </label>
                    </li>
                    <li class="image-upload__action image-upload__action--clear">
                        <svg width="29" height="25" viewBox="0 0 29 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M28.1504 23.2497H18.8379L27.8358 14.0983C28.5861 13.3353 28.9993 12.3209 28.9993 11.2419C28.9993 10.163 28.5861 9.14851 27.8358 8.38549L20.7742 1.20381C19.2255 -0.371247 16.7056 -0.371247 15.1569 1.20381L1.16351 15.4352C0.413193 16.1982 0 17.2126 0 18.2917C0 19.3706 0.413193 20.385 1.16351 21.1481L4.68044 24.7248C4.83977 24.8868 5.05586 24.9779 5.28117 24.9779H28.1504C28.6196 24.9779 29 24.591 29 24.1138C29 23.6366 28.6196 23.2497 28.1504 23.2497ZM13.3633 5.47178L15.5993 7.7458L8.74673 14.715L6.51073 12.441L13.3633 5.47178ZM20.2017 12.4265L13.3491 19.3956L9.94819 15.9369L16.8008 8.96775L20.2017 12.4265ZM21.4032 13.6484L23.6392 15.9225L21.3471 18.2538L16.7867 22.8916L14.5506 20.6175L21.4032 13.6484ZM16.3584 2.42576C17.2446 1.52455 18.6865 1.52455 19.5727 2.42576L26.6343 9.60744C27.5205 10.5087 27.5205 11.9752 26.6343 12.8764L24.8408 14.7005L14.5648 4.24984L16.3584 2.42576ZM2.36497 19.9261C1.93564 19.4895 1.69922 18.9089 1.69922 18.2917C1.69922 17.6743 1.93564 17.0938 2.36492 16.6572L5.30915 13.6629L8.14583 16.5478L8.14588 16.5479L12.7482 21.2285C12.7484 21.2287 12.7486 21.2288 12.7487 21.229L14.7357 23.2497H5.63308L2.36497 19.9261Z" fill="white"/>
                        </svg>
                    </li>
                </ul>
            </div>
        </div>
    @endif
</div>
