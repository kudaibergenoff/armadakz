<tr>
    ...
    <td class="products-table__image">
        <img src="{{ $additional->additional->getImages() }}" width="50" alt="Product image">
        <span class="hidden visibility-hidden">{{ env('APP_URL') }}{{ $product->getImages() }}</span>
    </td>
    <td>{{ $additional->additional->articul }}</td>
    <td>{{ $additional->additional->title }}</td>
    <td>
        <span class="font-weight-bold">{!! $additional->additional->item->title !!}</span>
        <span class="mx-2"><i class="fas fa-angle-double-right"></i></span>
        {!! $additional->additional->item->subcatalog->title !!}
        <span class="mx-2"><i class="fas fa-angle-double-right"></i></span>
        <span class="text-muted">{!! $additional->additional->item->subcatalog->catalog->title !!}</span>
    </td>
    <td>{{ $additional->additional->store->title }}</td>
    <td>
        <a href="{{ route('admin.product-additional.index') }}">{{ $additional->discount }}</a>
    </td>
    <td><span class="font-weight-semibold products-table__price">{{ $additional->additional->price }}</span></td>
    <td class="d-flex flex-column align-items-start">
        @if($additional->additional->isActive == true)
        ...
        @if($additional->additional->is_delivery == true)
            <div class="products__status products__status--five">Доставка</div>
        @endif
{{--        <div class="products__status products__status--hot">Горячее</div>--}}
{{--        <div class="products__status products__status--five">В пятерку</div>--}}
{{--        <div class="products__status products__status--active">Активный</div>--}}
    </td>
    <td style="white-space: nowrap">
        <b>Создания:</b><br>
        <span>{{ $additional->additional->created_at }}</span><br>
        <b>Обновления:</b><br>
        <span>{{ $additional->additional->updated_at }}</span>
    </td>
    <td class="text-center">
{{--        <button class="products-table__edit" data-toggle="modal" data-target="#quick-edit" data-item-id="{{ $product->id }}">--}}
{{--            <input type="hidden" name="edit-action" value="{{ route('admin.products.edit',['id'=>$product->id]) }}">--}}
{{--            <input type="hidden" name="edit-link" value="{{ route('admin.products.update',['id'=>$product->id]) }}">--}}
{{--            <input type="hidden" name="edit-product-isActive" value="{{ $product->isActive  }}">--}}
{{--            <input type="hidden" name="edit-product-isDelivery" value="{{ $product->is_delivery  }}">--}}
{{--            <input type="hidden" name="edit-product-price" value="{{ $product->price }}">--}}
...
{{--            </svg>--}}
{{--        </button>--}}
{{--        <button class="products-table__delete ml-2" data-toggle="modal" data-target="#confirm-delete-modal" data-item-id="{{ $product->id }}">--}}
{{--            <input type="hidden" name="delete-action" value="{{ route('admin.products.destroy',['id'=>$product->id]) }}">--}}
{{--            <input type="hidden" name="delete-product-name" value="{{ $product->title }}">--}}
{{--            <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">--}}
