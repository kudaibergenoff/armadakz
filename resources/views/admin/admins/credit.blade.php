            ...
            <div class="vendor__content flex-grow-1 vendor__content--products">
                ...
                <div class="orders vendor__shops pt-4 pb-3 p-xxl-3 pt-xxl-5 pr-0">
                    <div class="orders__header justify-content-between">
                        <h2 class="page-title orders__title">Изменить администратора</h2>
                    </div>
                    <form action="@isset($user){{ route('admin.admins.update',$user->id) }}@else{{ route('admin.admins.store') }}@endisset" class="pb-4 border-bottom" method="POST" enctype="multipart/form-data">
                        @isset($user) @method('PATCH') @endisset
                        @csrf
                        <div class="row align-items-center">
                            <div class="col-12 col-lg-3">
                                @include('admin.admins._uploadImage', ['data'=>$user,'name'=>'avatar','limit'=>1])
                            </div>
                            <div class="col-12 col-lg-9">
                                <div class="row">
                                    ...
                                    <div class="col-12 col-lg-6 mb-3">
                                        @include('layouts.forms.input',['name'=>'email','label'=>'E-mail','placeholder'=>'E-mail','value'=>($user->email ?? old('email')),'required'=>true])
                                    </div>
                                    <div class="col-12 col-lg-6 mb-3">
                                        @include('layouts.forms.input',['name'=>'password','type'=>'password','label'=>'Установить новый пароль','value'=>null,'placeholder'=>'Пароль'])
                                    </div>
                                    <div class="col-12 col-lg-6 mb-3">
                                        <label class="mdb-main-label">Роль по умолчанию</label>
...
{{--                                    <div class="col-12 col-lg-6 mb-3">--}}
{{--                                        <label class="mdb-main-label">Дополнительные роли</label>--}}
{{--                                        <select class="form-control js-example-basic-multiple" name="user_belongstomany_role_relationship[]" multiple>--}}
{{--                                            @foreach($roles as $role)--}}
{{--                                                <option value="{{ $role->id }}" @if($user->role_id == $role->id) selected @endif>{{ $role->title }}</option>--}}
{{--                                            @endforeach--}}
{{--                                        </select>--}}
{{--                                    </div>--}}
                                    <div class="col-12 col-lg-6">
                                        <label class="mdb-main-label">Язык</label>
                                        <select class="form-control js-example-basic-multiple" name="lang_id">
                                            <option value="" disabled selected>Выберите</option>
                                            @foreach($langs as $lang)
                                                <option value="{{ $lang->id }}" @if($user->lang_id == $lang->id) selected @endif>{{ $lang->title_ru }} ({{ $lang->slug }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
