<tr>
    ...
    <td style="min-width: 100px">
        @if($order->user_id != null) <i class="fas fa-user-tie"></i> {{ $order->userInfo->fio() }}<hr> @endif
...
{{--    <td>Мебельная фурнитура</td>--}}
    ...
    <td class="text-center" style="min-width: 200px">
        <a href="{{ route('admin.orders.edit',$order->id) }}" class="products-table__edit" data-item-id="{{ $order->id }}">
            <input type="hidden" name="edit-action" value="{{ route('admin.orders.edit',$order->id) }}">
            <input type="hidden" name="edit-link" value="{{ route('admin.orders.update',$order->id) }}">
            <input type="hidden" name="edit-product-isActive" value="{{ $order->status  }}">
        ...
        <form action="{{ route('admin.orders.update',$order->id) }}" method="POST" enctype="multipart/form-data" class="change-order-status">
            @method('PATCH')
            @csrf
