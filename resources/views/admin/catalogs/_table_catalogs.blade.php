<tr>
    <td></td>
    <td class="all-categories__main-item-icon">
        {!! $catalog->svg !!}
    </td>
    <td>
        @if($catalog->images != null)
            <img src="{{ $catalog->getImages() }}" alt="" width="75px">
        @endif
        {{ $catalog->title }}
    </td>
    <td>{{ $catalog->h1 }}</td>
    ...
    <td class="d-flex flex-column align-items-start">
        @if($catalog->isActive == 0)
            <div class="products__status products__status--hot">Не активный</div>
        @elseif($catalog->isActive == 1)
            <div class="products__status products__status--active">Активный</div>
        @endif

        @if($catalog->is_popular == 1)
            <div class="products__status products__status--hot">ТОП</div>
        @endif

        @if($catalog->is_menu == 1)
            <div class="products__status products__status--hot">В меню</div>
        @endif
    </td>
    <td>
        <a href="{{ route('admin.subcatalogs.index',$catalog->id) }}">
            <b>{{ $catalog->subcatalogs->count() }}</b>
        </a>
    </td>
    <td>{{ $catalog->index }}</td>
    <td>
        <b>Создания:</b><br>
        <span>{{ $catalog->created_at }}</span><br>
        <b>Обновления:</b><br>
        <span>{{ $catalog->updated_at }}</span>
    </td>
    <td class="text-center">
        <a href="{{ route('admin.catalogs.edit',$catalog->id) }}" class="products-table__edit" data-toggle="modal" data-target="#quick-edit" data-item-id="{{ $catalog->id }}">
            <input type="hidden" name="edit-action" value="{{ route('admin.catalogs.edit',$catalog->id) }}">
            <input type="hidden" name="edit-link" value="{{ route('admin.catalogs.update',$catalog->id) }}">
            <input type="hidden" name="edit-product-isActive" value="{{ $catalog->status  }}">
