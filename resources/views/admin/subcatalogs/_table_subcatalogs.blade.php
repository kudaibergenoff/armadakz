<tr>
    <td></td>
    <td>
        @if($subcatalog->images != null)
            <img src="{{ $subcatalog->getImages() }}" alt="" width="75px">
        @endif
        {{ $subcatalog->title }}
    </td>
    <td>{{ $subcatalog->h1 }}</td>
    <td>{{ $subcatalog->slug }}</td>
    <td>
        <a href="{{ route('admin.subcatalogs.index',['catalog_id'=>$subcatalog->catalog->id]) }}">
            {{ $subcatalog->catalog->title }}
        </a>
    </td>
    <td class="d-flex flex-column align-items-start">
        @if($subcatalog->isActive == 0)
            <div class="products__status products__status--hot">Не активный</div>
        @elseif($subcatalog->isActive == 1)
            <div class="products__status products__status--active">Активный</div>
        @endif

        @if($subcatalog->is_popular == 1)
            <div class="products__status products__status--hot">ТОП</div>
        @endif
    </td>
    <td>
        <a href="{{ route('admin.items.index',['subcatalog_id'=>$subcatalog->id]) }}">
            <b>{{ $subcatalog->items->count() }}</b>
        </a>
    </td>
    <td>
        <b>Создания:</b><br>
        <span>{{ $subcatalog->created_at }}</span><br>
        <b>Обновления:</b><br>
        <span>{{ $subcatalog->updated_at }}</span>
    </td>
    <td class="text-center">
        <a href="{{ route('admin.subcatalogs.edit',$subcatalog->id) }}" class="products-table__edit" data-toggle="modal" data-target="#quick-edit" data-item-id="{{ $subcatalog->id }}">
            <input type="hidden" name="edit-action" value="{{ route('admin.subcatalogs.edit',$subcatalog->id) }}">
            <input type="hidden" name="edit-link" value="{{ route('admin.subcatalogs.update',$subcatalog->id) }}">
            <input type="hidden" name="edit-product-isActive" value="{{ $subcatalog->isActive }}">
