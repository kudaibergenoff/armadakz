...
{{--            <span class="text-muted">{{ $item->subcatalog->catalog->title }}</span><br>--}}
    ...
    <td class="text-center">
        <a href="{{ route('admin.news.edit',$new->id) }}" class="products-table__edit" > <!-- data-toggle="modal" data-target="#quick-edit" data-item-id="{{ $new->id }}" -->
{{--            <input type="hidden" name="edit-action" value="{{ route('admin.news.edit',['id'=>$new->id]) }}">--}}
{{--            <input type="hidden" name="edit-link" value="{{ route('admin.newsUpdate',['id'=>$new->id]) }}">--}}
{{--            <input type="hidden" name="edit-product-isActive" value="{{ $new->isActive  }}">--}}
        ...
        <button class="products-table__delete ml-2" data-toggle="modal" data-target="#confirm-delete-modal" data-item-id="{{ $new->id }}">
            <input type="hidden" name="delete-action" value="{{ route('admin.news.destroy',$new->id) }}">
            <input type="hidden" name="delete-product-name" value="{{ $new->title }}">
            <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
