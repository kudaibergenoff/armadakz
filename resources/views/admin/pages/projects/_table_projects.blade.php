<tr>
    <td><img src="{{ $project->getImage('images') }}" alt="" height="100px"></td>
    <td>{{ $project->title }}</td>
    <td>{{ $project->slug }}</td>
    ...
    <td class="white-space-nowrap">
        <b>Создания:</b><br>
        <span>{{ $project->created_at }}</span><br>
        <b>Обновления:</b><br>
        <span>{{ $project->updated_at }}</span>
    </td>
    <td class="text-center">
        <a href="{{ route('admin.projects.edit',$project->id) }}" class="products-table__edit" data-toggle="modal" data-target="#quick-edit" data-item-id="{{ $project->id }}">
            <input type="hidden" name="edit-action" value="{{ route('admin.projects.edit',$project->id) }}">
            <input type="hidden" name="edit-link" value="{{ route('admin.projects.update',$project->id) }}">
            <input type="hidden" name="edit-product-isActive" value="{{ $project->isActive  }}">
