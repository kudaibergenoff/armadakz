<tr>
    <td></td>
    <td>
        <img src="{{ 'https://img.youtube.com/vi/'. $video->img .'/0.jpg' }}" alt="" height="100px">
    </td>
    ...
    <td>
        @if($video->is_active == true)
            <div class="products__status products__status--active">Активно</div>
        @else
            <div class="products__status products__status--disabled">Не активно</div>
        @endif
    </td>

    <td class="text-center">
        <a href="{{ route('admin.videos.edit',$video->id) }}" class="products-table__edit" data-item-id="{{ $video->id }}">
            <input type="hidden" name="edit-action" value="{{ route('admin.videos.edit',$video->id) }}">
            <input type="hidden" name="edit-link" value="{{ route('admin.videosUpdate',$video->id) }}">
            <input type="hidden" name="edit-product-isActive" value="{{ $video->is_фctive  }}">
        ...
        <button class="products-table__delete ml-2" data-toggle="modal" data-target="#confirm-delete-modal" data-item-id="{{ $video->id }}">
            <input type="hidden" name="delete-action" value="{{ route('admin.videos.destroy',$video->id) }}">
            <input type="hidden" name="delete-product-name" value="{{ $video->title }}">
            <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
