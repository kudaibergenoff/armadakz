<tr>
    ...
    <td class="products-table__image">
        <img src="{{ $product->getImages() }}" width="50" alt="Product image">
        <span class="hidden visibility-hidden">{{ env('APP_URL') }}{{ $product->getImages() }}</span>
    </td>
    <td>{{ $product->articul }}</td>
    <td>{{ $product->title }}</td>
    <td>
        <span class="font-weight-bold">{!! $product->item->title !!}</span>
        <span class="mx-2"><i class="fas fa-angle-double-right"></i></span>
        {!! $product->item->subcatalog->title !!}
        <span class="mx-2"><i class="fas fa-angle-double-right"></i></span>
        <span class="text-muted">{!! $product->item->subcatalog->catalog->title !!}</span>
    </td>
    <td>{{ $product->store->title }}</td>
    <td>
        <a href="{{ route('admin.additional.index',['product_id'=>$product->id]) }}" class="text-primary text-center font-weight-bold">
            {{ $product->additional_count }}
        </a>
    </td>
    <td><span class="font-weight-semibold products-table__price">{{ $product->price }}</span></td>
    <td class="d-flex flex-column align-items-start">
        ...
        @if($product->is_hot == true)
            <div class="products__status products__status--disabled">Горячий</div>
        @endif

        <div class="products__status products__status--{{ $product->presence->color ?? 'hot' }}">{{ $product->presence->title }}</div>
...
{{--        <div class="products__status products__status--active">Активный</div>--}}
    ...
    <td style="white-space: nowrap">
        <b>Создания:</b><br>
        <span>{{ $product->created_at }}</span><br>
        <b>Обновления:</b><br>
        <span>{{ $product->updated_at }}</span>
    </td>
    <td class="text-center">
        <a href="{{ route('admin.products.edit',$product->id) }}" class="products-table__edit" > <!-- data-toggle="modal" data-target="#quick-edit" data-item-id="{{ $product->id }}" -->
...
{{--            <input type="hidden" name="edit-product-price" value="{{ $product->price }}">--}}
        ...
        <button class="products-table__delete ml-2" data-toggle="modal" data-target="#confirm-delete-modal" data-item-id="{{ $product->id }}">
            <input type="hidden" name="delete-action" value="{{ route('admin.products.destroy',$product->id) }}">
