<tr>
    ...
    <td>{{ $review->id }}</td>
    <td>
        <a href="{{ route('product',['slug'=>$review->product->slug]) }}" target="_blank">{!! $review->product->title !!}</a><br>
        {!! $review->store->title !!}
    </td>
    <td>
        {{ $review->name }}<br>
    ...
    <td style="max-width: 550px; word-wrap: break-word">{{ $review->comment }}</td>
    <td>
    ...
    <td class="text-center" style="width: 200px">
        <form action="{{ route('admin.reviews.update',$review->id) }}" method="POST" enctype="multipart/form-data" class="change-order-status">
            @method('PATCH')
            @csrf
