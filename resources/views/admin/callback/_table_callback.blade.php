<tr>
    ...
    <td class="text-center">
        @if($callback->product_id != null)
            <a href="{{ route('product',[$callback->product->id ?? '',$callback->product->slug ?? '']) }}" target="_blank">{{ $callback->product->title }}</a>
        @endif
    </td>
    <td class="text-center">{{ $callback->name ?? '' }}</td>
    <td class="text-center">{{ $callback->phone ?? '' }}</td>
    <td class="text-center">
        {{ $callback->created_at->format('d.m.Y') }}<br>
        {{ $callback->created_at->format('h:i') }}
        {{--        <a href="{{ route('admin.colors.edit',$color->id) }}" class="products-table__edit" >--}}
        ...
        {{--            </svg>--}}
        {{--        </a>--}}
        {{--        <button class="products-table__delete"  data-toggle="modal" data-target="#confirm-delete-modal" data-item-id="{{ $color->id }}">--}}
        {{--            <input type="hidden" name="delete-action" value="{{ route('admin.colors.destroy',['id'=>$color->id]) }}">--}}
        {{--            <input type="hidden" name="delete-product-name" value="{{ $color->title_ru }}">--}}
        {{--            <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">--}}
        ...
        {{--            </svg>--}}
        {{--        </button>--}}
        <div class="products-table__delete" data-item-id="{{ $callback->id }}"></div>
    </td>
</tr>
