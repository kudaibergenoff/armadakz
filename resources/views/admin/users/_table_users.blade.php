<tr>
    <td></td>
    <td><img src="{{ $user->getImage('avatar') }}" alt="" width="100px"></td>
    <td>{!! $user->fio() !!}</td>
    <td>{{ $user->email }}</td>
    <td>
    ...
    <td>
        <div class="products__status products__status--{{ $user->status->color }}">{{ $user->status->title }}</div>
{{--        <div class="products__status products__status--five">Доставка</div>--}}
    ...
    <td class="text-center">
        <a href="{{ route('admin.orders.index',$user->id) }}"><b>{{ $user->orders_count }}</b></a><br>
    </td>
    <td class="text-center">
        @isCan('edit_users')
        <a href="{{ route('admin.users.edit',$user->id) }}" class="products-table__edit">
...
{{--            <input type="hidden" name="delete-action" value="{{ route('admin.users.destroy',$user->id) }}">--}}
