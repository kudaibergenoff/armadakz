<tr>
    ...
    <td>
        @if($item->images != null)
            <img src="{{ $item->getImages() }}" alt="" width="75px">
        @endif
    </td>
    <td>{{ $item->title }}</td>
    <td>{{ $item->h1 }}</td>
    <td>{{ $item->slug }}</td>
    <td>
        <a href="{{ route('admin.items.index',['subcatalog_id'=>$item->subcatalog->id]) }}">
            {{ $item->subcatalog->title }}
        </a>
    </td>
    <td>
        <a href="{{ route('admin.items.index',['catalog_id'=>$item->subcatalog->catalog->id]) }}">
            {{ $item->subcatalog->catalog->title }}
        </a>
    </td>
    ...
    <td class="text-center">
        <a href="{{ route('admin.items.edit',$item->id) }}" class="products-table__edit" data-toggle="modal" data-target="#quick-edit" data-item-id="{{ $item->id }}">
            <input type="hidden" name="edit-action" value="{{ route('admin.items.edit',$item->id) }}">
            <input type="hidden" name="edit-link" value="{{ route('admin.items.update',$item->id) }}">
            <input type="hidden" name="edit-product-isActive" value="{{ $item->isActive }}">
