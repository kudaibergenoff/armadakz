        ...
        <div class="d-flex">
            ...
            <div class="vendor__content flex-grow-1 vendor__content--products">
                ...
                <div class="shops orders vendor__shops pt-4 pb-3 p-xxl-3 pt-xxl-5 pr-0">
                    ...
                    <form action="@isset($data){{ route('admin.items.update',$data->id) }}@else{{ route('admin.items.store') }}@endisset" class="change-shop pb-4 border-bottom was-validated" method="POST"  enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-lg-4 mb-3">
                                ...
                                <select class="form-control js-example-basic-single" name="subcatalog_id" required><!-- multiple="multiple" -->
                                    @foreach($catalogs as $catalog)
                                        <optgroup label="{{ $catalog->title }}">
                                            @foreach($subcatalogs->where('catalog_id',$catalog->id) as $subcatalog)
                                                <option value="{{ $subcatalog->id }}" @if(isset($data) and $data->subcatalog_id == $subcatalog->id) selected @endif>{{ $subcatalog->title }}</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                </select>
