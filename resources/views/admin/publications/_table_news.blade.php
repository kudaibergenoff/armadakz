<tr>
    <td></td>
    <td><img src="{{ $new->getImage('images') }}" alt="" height="100px"></td>
    <td>{{ $new->type->title }}</td>
{{--    <td>--}}
{{--        @foreach($new->items as $item)--}}
{{--            <span class="font-weight-bold">{{ $item->title }}</span>--}}
{{--            <span class="mx-2"><i class="fas fa-angle-double-right"></i></span>--}}
{{--            {{ $item->subcatalog->title }}--}}
{{--            <span class="mx-2"><i class="fas fa-angle-double-right"></i></span>--}}
{{--            <span class="text-muted">{{ $item->subcatalog->catalog->title }}</span><br>--}}
{{--        @endforeach--}}
{{--    </td>--}}
    <td>
        {{ $new->store->title ?? '' }}
    ...
    <td>
        ...
        {{ $new->description ?? '' }}
        <hr class="my-1">
        <div class="text-crop" data-crop-size="200">{!! $new->text !!}</div>
    </td>
    <td>
        <div class="products__status products__status--{{ $new->status->color }}">{{ $new->status->title }}</div>
    </td>
    <td class="text-center">
        @isCan('edit_publications')
        <a href="{{ route('admin.publications.edit',$new->id) }}" class="products-table__edit" >
{{--            data-toggle="modal" data-target="#quick-edit" data-item-id="{{ $new->id }}"--}}
{{--            <input type="hidden" name="edit-action" value="{{ route('seller.news.edit',['id'=>$new->id]) }}">--}}
{{--            <input type="hidden" name="edit-link" value="{{ route('seller.newsUpdate',['id'=>$new->id]) }}">--}}
{{--            <input type="hidden" name="edit-product-isActive" value="{{ $new->isActive  }}">--}}
