<tr>
    ...
    <td>
        {!! $user->company ?? '<span class="text-danger font-weight-bold">Нет данных !!!</span>' !!}
        <hr class="my-1">
        {!! $user->fio() !!}
    </td>
    <td>
        @if($user->stores->count() > 0)
            @foreach($user->stores as $store)
                <a href="shops/{{ $store->slug }}" class="d-block" target="_blank">{{ $store->title }}</a>
            @endforeach
        @else
...
{{--        <div class="products__status products__status--five">Доставка</div>--}}
    ...
    <td>{{ $user->seller_device_info }}</td>
    <td>
        <b>Регистрации:</b><br>
        <span>{{ $user->created_at }}</span><br>
        <b>Обновления:</b><br>
        <span>{{ $user->updated_at }}</span><br>
        <b>Последней активности:</b><br>
